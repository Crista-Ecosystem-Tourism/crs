...
| API через фронт | [http://localhost:3333/api/health](http://localhost:3333/api/health) |
...
| ------ | --- | ---------- |
| Frontend | [https://crista.online](https://crista.online) | UI; фронтовый Docker build ходит к API через `/api` и `/graph-api`, чтобы не ловить CORS |
